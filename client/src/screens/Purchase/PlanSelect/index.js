...
/*
  ...
  useEffect( async () => {
    if (isUpgradeFlow) {
      if (currentPlan == premium_plan) {
        ...
        await window.intempt.configure(
          ...
          console.log('Before recording event');
           await window.intempt.recordEvent('subscribed_to_premium_plan', {
            subscribed:"data",
            total_amount: 299
          });
    ...
    await window.intempt.configure(
     ...
     await window.intempt.recordEvent('subscribed_to_basic_plan', {
        subscribed: "data",
        total_amount: 99
  ...
  const selectPlan = async (plan, price, type) => {
    setPlan(plan);
    setPrice(price);
    setPlanType(type);
  console.log("plan tell me please",plan)
  ...
  const submitPlan = async() => {
    ...
    if (plan.includes("price_1N40B2HkG8KztjeqZNIvzfKT")) {
     await window.intempt.recordEvent('subscribed_to_basic_plan', {
        subscribed: "data",
        total_amount: 99
    ...
    } else if (plan.includes("price_1N5VVqHkG8KztjeqMeDzUsol")) {
      await window.intempt.recordEvent('subscribed_to_premium_plan', {
        subscribed: "data",
        total_amount: 299
